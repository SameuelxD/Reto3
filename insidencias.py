import core
import os
TipoInsidencias=["HARDWARE","SOFTWARE"]
AreasGenerales={
    "AREA TRAINING":["APOLO","ARTEMIS","SPUTNIK","SKYLAB"],
    "AREA REVIEWS":["AREA REVIEWS1","AREA REVIEWS2"]
}
CategoriaInsidencias=["LEVE","MODERADA","CRITICA"]
infoInsidencias= {"data":[]}
def LoadInfoInsidencias():
    global infoInsidencias
    if (core.CheckFile("insidencias.json")):
        infoInsidencias = core.LoadInfo("insidencias.json")
    else:
        core.AddInfo("insidencias.json",infoInsidencias)
def MenuInsidencias():
    os.system("cls")
    Passive=True 
    print('+','-'*55,'+')
    print("|{:^16}{}{:^15}|".format(' ','GESTION DE INSIDENCIAS',' '))
    print('+','-'*55,'+')
    ...
    if (opcion == 1):
        while(Passive):
            os.system("cls")
            print('+','-'*49,'+')
            print("|{:^16}{}{:^15}|".format(' ','AGREGAR INSIDENCIA',' '))
            print('+','-'*49,'+')
            Pause=True
            while(Pause):
                try:
